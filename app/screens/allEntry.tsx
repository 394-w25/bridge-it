...
const CATEGORIES = [
  { name: 'Academic', color: '#FDE68A' },
  { name: 'Personal', color: '#99E9F2' },
  { name: 'Leadership', color: '#F8B4C0' },
  { name: 'Research', color: '#BBF7D0' },
  { name: 'Project', color: '#FDAF75' },
];
...
const AllEntriesModal: React.FC<AllEntriesProps> = ({ visible, onClose }) => {
  ...
  useEffect(() => {
    if (uid) {
      getUserEntries(uid).then((entries) => {
        const formattedEntries = entries.map(entry => ({
          title: entry.title || "Untitled",
          shortSummary: entry.shortSummary || "No short summary available",
          timestamp: entry.timestamp.toDate().toISOString(),
      ...
      <View style={styles.modalOverlay}>
        <LinearGradient colors={['#FFF6C8', '#FFFFFF']} style={styles.container}>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity onPress={() => openEntryModal(item)} style={styles.entryCard}>
                <Text style={styles.entryTitle}>{item.title}</Text>
                <Text style={styles.entrySummary}>{item.shortSummary}</Text>
              </TouchableOpacity>
            )}
...
const styles = StyleSheet.create({
  ...
  entriesContainer: { paddingHorizontal: 16 },
  entryCard: { paddingVertical: 16, borderBottomWidth: 1, borderBottomColor: '#ddd' },
  entryTitle: { fontSize: 16, fontWeight: 'bold' },
  entrySummary: { fontSize: 14, color: "#555" },
  
  entryModalOverlay: { flex: 1, justifyContent: 'center', backgroundColor: 'rgba(0,0,0,0.2)' },
  entryModalContainer: { flex: 1, backgroundColor: '#FFFFFF', padding: 20 },
